...
class AppFixtures extends Fixture
{
    ...
    {
        foreach (self::CATALOG as $nomCategorie => $produits) {
            ...
            foreach ($produits as $donneesProduit) {
                [$nom, $description, $prix, $produitType, $imageUrl] = $donneesProduit;

                $produit = new Produit();
                $produit->setNom($nom);
                $produit->setDescription($description);
                $produit->setPrix((float) $prix);
                $produit->setStock(rand(10, 100)); // Stock aléatoire entre 10 et 100
                $produit->setImageUrl($imageUrl);
                $produit->setAnimalType($nomCategorie);
                $produit->setProduitType($produitType);
                $produit->setCreatedAt(new \DateTimeImmutable());
                $produit->setIsPromo(false);
